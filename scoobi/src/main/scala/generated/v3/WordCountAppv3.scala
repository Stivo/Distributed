...
object WordCountApp extends ScoobiApp {
  ...
  def run() {
    ...
    val x1699 = x3.parallelDo(new DoFn[java.lang.String, scala.Tuple2[java.lang.String, Int], Unit] {
      lazy val x5 = new ch.epfl.distributed.datastruct.RegexFrontend("""	""", false, true);
      lazy val x23 = new ch.epfl.distributed.datastruct.RegexFrontend("""\[\[.*?\]\]""", false, true);
      lazy val x28 = new ch.epfl.distributed.datastruct.RegexFrontend("""(\\[ntT]|\.)\s*(thumb|left|right)*""", false, true);
      lazy val x33 = new ch.epfl.distributed.datastruct.RegexFrontend("""[^a-zA-Z0-9']+""", false, true);
      lazy val x44 = new ch.epfl.distributed.datastruct.RegexFrontend("""(thumb|left|right|\d+px){2,}""", false, true);
      def setup(): Unit = {}
