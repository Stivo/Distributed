...
object WordCountApp extends ScoobiApp {
  ...
  def run() {
    ...
    val x1699 = x3.parallelDo(new DoFn[java.lang.String, scala.Tuple2[java.lang.String, Int], Unit] {
      var x5: RegexFrontend = null
      var x23: RegexFrontend = null
      var x28: RegexFrontend = null
      var x33: RegexFrontend = null
      var x44: RegexFrontend = null
      def setup(): Unit = {
        x5 = new ch.epfl.distributed.datastruct.RegexFrontend("""	""", true, true);
        x23 = new ch.epfl.distributed.datastruct.RegexFrontend("""\[\[.*?\]\]""", true, true);
        x28 = new ch.epfl.distributed.datastruct.RegexFrontend("""(\\[ntT]|\.)\s*(thumb|left|right)*""", true, true);
        x33 = new ch.epfl.distributed.datastruct.RegexFrontend("""[^a-zA-Z0-9']+""", true, true);
        x44 = new ch.epfl.distributed.datastruct.RegexFrontend("""(thumb|left|right|\d+px){2,}""", true, true);
      }
