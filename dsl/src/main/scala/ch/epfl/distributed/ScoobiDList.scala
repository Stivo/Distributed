...
trait ScoobiGenDList extends ScalaGenBase
    ...
    with DListTransformations with Matchers with SwitchingTypeFactory {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = {
    val out = rhs match {
      ...
      case vs @ DListSave(dlist, filename) => stream.println("persist(TextOutput.toTextFile(%s,%s))".format(quote(dlist), quote(filename)))
